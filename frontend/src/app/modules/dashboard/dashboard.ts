...
export class Dashboard {
  ...
  private parseExcel(data: ArrayBuffer): void {
    ...
    if (!expectedHeaders.every((header) => headers.includes(header))) {
      throw new Error(
        'Invalid Excel format. Please use the correct column headers.'
      );
    }
  ...
  private parseCSV(csvContent: string): void {
    ...
    if (!expectedHeaders.every((header) => headers.includes(header))) {
      throw new Error(
        'Invalid CSV format. Please use the correct column headers.'
      );
    }
  ...
  private processStudentData(rows: any[][], headers: string[]): void {
    ...
    if (newStudentScores.length === 0) {
      this.errorMessage = 'No valid student data found in the file';
      this.hasValidationErrors = true;
      return;
    }

    if (duplicateNames.size > 0) {
      this.errorMessage = `Duplicate entries found for: ${Array.from(
        duplicateNames
      ).join(', ')}`;
    }

    this.studentScoresList = newStudentScores;
    this.fileUploaded = !this.hasValidationErrors;

    if (this.hasValidationErrors) {
      this.errorMessage =
        this.errorMessage ||
        'Validation errors found in the file. Please correct and try again.';
    }
  }
